...
///   <item><c>Email:SenderAddress</c> — From address, default <c>noreply@realmunbound.com</c>.</item>
///   <item><c>Email:SenderName</c> — From display name, default <c>RealmUnbound</c>.</item>
/// </list>
...
public class SmtpEmailSender(IConfiguration config, ILogger<SmtpEmailSender> logger) : IEmailSender
{
    ...
    public async Task SendAsync(string to, string subject, string htmlBody, CancellationToken ct = default)
    {
        ...
        var senderAddress  = config["Email:SenderAddress"] ?? "noreply@realmunbound.com";
        var senderName     = config["Email:SenderName"]    ?? "RealmUnbound";
