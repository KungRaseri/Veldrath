...
public class AuthEndpointTests(WebAppFactory factory)
{
    ...
    public async Task Register_Should_Return_Token_Pair()
    {
        var response = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_register_user@test.com", Username = "Auth_Register_User", Password = "TestP@ssword123" });
    ...
    public async Task Register_Should_Reject_Duplicate_Username()
    {
        await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_dupe_user@test.com", Username = "Auth_Dupe_User", Password = "TestP@ssword123" });

        var second = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_dupe_user2@test.com", Username = "Auth_Dupe_User", Password = "TestP@ssword123" });
    ...
    {
        await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_login_user@test.com", Username = "Auth_Login_User", Password = "TestP@ssword123" });

        var response = await _client.PostAsJsonAsync("/api/auth/login",
            new { Email = "auth_login_user@test.com", Password = "TestP@ssword123" });
    ...
    {
        await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_badpass_user@test.com", Username = "Auth_BadPass_User", Password = "TestP@ssword123" });
    ...
    {
        var response = await _client.PostAsJsonAsync("/api/auth/login",
            new { Email = "nosuchuser@test.com", Password = "TestP@ssword123" });
    ...
    public async Task Refresh_Should_Issue_New_Token_Pair()
    {
        var reg = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_refresh_user@test.com", Username = "Auth_Refresh_User", Password = "TestP@ssword123" });
    ...
    {
        var reg = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_theft_user@test.com", Username = "Auth_Theft_User", Password = "TestP@ssword123" });
        var auth = await reg.Content.ReadFromJsonAsync<AuthResponse>();
    ...
    {
        var reg = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_logout_user@test.com", Username = "Auth_Logout_User", Password = "TestP@ssword123" });
        var auth = await reg.Content.ReadFromJsonAsync<AuthResponse>();
    ...
    {
        await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "dupe_email@test.com", Username = "Auth_DupeEmail_User1", Password = "TestP@ssword123" });

        var second = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "dupe_email@test.com", Username = "Auth_DupeEmail_User2", Password = "TestP@ssword123" });
    ...
    public async Task Register_Should_Return_Correct_Username_In_Response()
    {
        ...
        var response = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_username_return@test.com", Username = username, Password = "TestP@ssword123" });
    ...
    {
        var reg = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_preserve_account@test.com", Username = "Auth_PreserveAcct_User", Password = "TestP@ssword123" });
    ...
    {
        var reg = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_same_accountid@test.com", Username = "Auth_SameAcctId_User", Password = "TestP@ssword123" });
        var registered = await reg.Content.ReadFromJsonAsync<AuthResponse>();

        var login = await _client.PostAsJsonAsync("/api/auth/login",
            new { Email = "auth_same_accountid@test.com", Password = "TestP@ssword123" });
    ...
    {
        var reg = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_rotate_check@test.com", Username = "Auth_RotateCheck_User", Password = "TestP@ssword123" });
        var original = await reg.Content.ReadFromJsonAsync<AuthResponse>();
    ...
    {
        var response = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_expiry@test.com", Username = "Auth_Expiry_User", Password = "TestP@ssword123" });
    ...
    public async Task Refresh_Should_Return_Unauthorized_For_Expired_Token()
    {
        ...
        var reg = await _client.PostAsJsonAsync("/api/auth/register",
            new { Email = "auth_expired_refresh@test.com", Username = "Auth_ExpiredRefresh_User", Password = "TestP@ssword123" });
        var auth = await reg.Content.ReadFromJsonAsync<AuthResponse>();
