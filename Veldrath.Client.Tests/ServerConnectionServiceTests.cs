...
public class ServerConnectionServiceTests : TestBase
{
    ...
    // ConnectAsync guard — Connecting / Reconnecting states
    [Fact]
    ...
    // ConnectAsync retry — disposal of failed connection
    [Fact]
    ...
    // ConnectAsync retry — stale Closed event from previous failed connection is ignored
    [Fact]
    public async Task ConnectAsync_Should_Ignore_Closed_Event_From_Previous_Failed_Connection()
    {
        ...
        // Stale Closed from the old (failed) connection — must be suppressed
        await factory.AllCreated[0].SimulateClosedAsync();

        connectionLostFired.Should().BeFalse();
    }

    // DisconnectAsync — full disposal
    [Fact]
    ...
    // DisconnectAsync — Closed event after intentional stop must not fire ConnectionLost
    [Fact]
    public async Task DisconnectAsync_Should_Not_Fire_ConnectionLost()
    {
        ...
        await svc.DisconnectAsync();
        // Simulate SignalR firing Closed after intentional stop — must be suppressed
        await factory.Connection.SimulateClosedAsync();
